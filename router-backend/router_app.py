...
class RouterApp:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send):
        ...
        if path.startswith("/email"):
            await self.email(scope, receive, send)
            return
...
# Email app (Static Files) - Disabled
# email_static_path = BASE_DIR / "router-backend" / "static" / "email"
# if email_static_path.exists():
#     email_asgi_app = StaticFiles(directory=str(email_static_path), html=True)
#     app.mount("/email", email_asgi_app, name="email")
#     logger.info("✓ Email app mounted at /email (Static Files)")
# else:
#     logger.error("✗ Email static files not found at {email_static_path}")
#     # Let the router handle 404s naturally instead of showing placeholder
#     email_asgi_app = FastAPI()
#     app.mount("/email", email_asgi_app, name="email")

# Email app (Static Files) - Load from built mail app
try:
    from pitext_mail.email_app import email_app as email_asgi_app
    app.mount("/email", email_asgi_app, name="email")
    logger.info("✓ Email app mounted at /email (Static Files)")
except Exception as e:
    logger.error(f"✗ Failed to mount email app: {e}")
    import traceback
    traceback.print_exc()
    # Let the router handle 404s naturally instead of showing placeholder
    email_asgi_app = FastAPI()
